...
class ValidationServiceSpec extends UnitSpec {
  ...
  given CanEqual[Either[Seq[ThirdPartyResponseError], ThirdPartyPayRequest], Either[Seq[
  ...
  "validateThirdPartyRequest" - {
    ...
    "return a Right[ThirdPartyPayRequest] when jsValue is valid" in {
      val result = service.validateThirdPartyRequest(
        Json.parse(
          """{"taxRegime":"SelfAssessment","reference":"1234567895","amountInPence":123,"friendlyName":"Test Company","backURL":"https://valid-url.com"}"""
        )
      )
  ...
  "jsErrorToMessagesBetter" - {
    "should take the json key and prepend it to a relevant json validation error key" in {
      val input: collection.Seq[(JsPath, collection.Seq[JsonValidationError])] =
    ...
    "should take the json key and prepend it to all relevant json validation error keys" in {
      val input: collection.Seq[(JsPath, collection.Seq[JsonValidationError])] =
        collection.Seq[(JsPath, collection.Seq[JsonValidationError])](
          JsPath(List[PathNode](KeyPathNode("/something"))) -> collection.Seq(
            JsonValidationError("some.error.message"),
            JsonValidationError("some.other.error.message"),
            JsonValidationError("yet.another.error.message")
          )
        )
      val result                                                               = service.jsErrorToMessagesBetter(input)
      result shouldBe Seq(
        "something.some.error.message",
        "something.some.other.error.message",
        "something.yet.another.error.message"
      )
    }
  }
