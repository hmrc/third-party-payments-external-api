...
import play.api.libs.json.{JsError, JsNumber, JsString, JsSuccess, Json, JsonValidationError}
...
class ThirdPartyPayRequestSpec extends UnitSpec {
  ...
  "ThirdPartyPayRequest" - {
    ...
    val testClientJourneyId: ClientJourneyId = ClientJourneyId(UUID.fromString("aef0f31b-3c0f-454b-9d1f-07d549987a96"))
      ...
      def jsValue(taxRegimeString: String) = Json.parse(s"""{"taxRegime":"$taxRegimeString","reference":"someReference","amountInPence":123,"friendlyName":"Test Company","backURL":"https://valid-url.com"}""")
    ...
    "friendlyName.reads" - {
      ...
      "cause a JsError when friendly name string contains a special character not in the allowed list" in {
        ...
        FriendlyName.reads.reads(JsString(stringContainingInvalidCharacter)) shouldBe JsError(JsonValidationError(List("error.invalidCharacters")))
      }
      ...
      "cause a JsError when friendly name string is too long (more than 40 characters)" in {
        ...
        FriendlyName.reads.reads(JsString(stringMoreThan40Characters)) shouldBe JsError(JsonValidationError(List("error.maxLength")))
      }
    }
    ...
    "amountInPence.reads" - {
      ...
      "return a JsError when amount is less than 0" in {
        ...
        AmountInPence.reads.reads(JsNumber(negativeAmount)) shouldBe JsError(JsonValidationError(List("error.minimumValue")))
      }
    }
    ...
    "taxRegime.reads" - {
      ...
      "return a JsError when amount is less than 0" in {
        ...
        TaxRegime.keyReads.readKey(invalidTaxRegime) shouldBe JsError(JsonValidationError(List("error.expected.validenumvalue")))
      }
    ...
    "asSaSpjRequest correctly creates SpjRequest3psSa" in {
      val spjRequest = SpjRequest3psSa(Reference("someReference"), AmountInPence(123), testClientJourneyId, Some(FriendlyName("Test Company")), Some(URL("https://valid-url.com")), Some(URL("https://valid-url.com")))
    ...
    "asVatSpjRequest correctly creates SpjRequest3psVat" in {
      val spjRequest = SpjRequest3psVat(Reference("someReference"), AmountInPence(123), testClientJourneyId, Some(FriendlyName("Test Company")), Some(URL("https://valid-url.com")), Some(URL("https://valid-url.com")))
    ...
    "asCorporationTaxSpjRequest correctly creates SpjRequest3psCorporationTax" in {
      val spjRequest = SpjRequest3psCorporationTax(Reference("someReference"), AmountInPence(123), testClientJourneyId, Some(FriendlyName("Test Company")), Some(URL("https://valid-url.com")), Some(URL("https://valid-url.com")))
    ...
    "asEmployersPayAsYouEarnSpjRequest correctly creates SpjRequest3psEmployersPayAsYouEarn" in {
      val spjRequest = SpjRequest3psEmployersPayAsYouEarn(Reference("someReference"), AmountInPence(123), testClientJourneyId, Some(FriendlyName("Test Company")), Some(URL("https://valid-url.com")), Some(URL("https://valid-url.com")))
